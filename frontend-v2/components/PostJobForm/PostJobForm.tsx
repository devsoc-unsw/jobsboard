...
const PostJobForm = ({ admin }: Props) => {
  ...
  useEffect(() => {
    const setup = async () => {
      ...
      if (res.status === 200) {
        ...
        setVerifiedCompanies(res.data.companies.sort(
  ...
  const submitJobPost = async () => {
    ...
    let jobDate = new Date(expiryDate);
    ...
    if (
      ...
      isNaN(parseInt(selectedCompanyID, 10))
    ) {
      // error message
      setAlertType('error')
    ...
    if (res.status === 200) {
      ...
      setAlertMsg('Job posted! This job will be made available to students shortly. Redirecting to your dashboard...')
      setAlertOpen(true)
      window.scrollTo({
        top: 0,
        behavior: 'smooth',
      ...
      if (res.status === 403) {
        ...
        setAlertMsg('Looks like something went wrong. Please ensure that all fields are filled with a valid value.')
      }
      setAlertOpen(true)
      window.scrollTo({
        top: 0,
  ...
  return (
    <div>
        ...
        {/* <!-- disclaimer box --> */}
      {
        !admin && alertOpen && 
        <div
          className='bg-orange-100 border-t-4 border-orange-500 rounded-b px-4 py-3 shadow-md mx-[15%] lg:mx-0 mb-10'
        >
          <div className='flex'>
            <div className='py-1'>
              <FontAwesomeIcon
        ...
        >
          ...
          {Object.values(verifiedCompanies).map((company) =><option key={company.id} value={company.id}>
