    ...
    <div>
      ...
      <div className='flex flex-col px-8 items-center'>
        ...
        {companies.map((company) => <PendingCompanyCard
          ...
          onClick={() => triggerModal(company.company.name, company.company.location, company.company.description)}
          onAlert={triggerAlert}
          onRemove={() => setCompanies((prevState) => prevState.filter((c) => c.id !== company.id))}
        />)}
      </div>
    </div>
  )
