...
describe('create job post as a unofficial company while logged in as an admin', () => {
  before(async function () {
    ...
    await server.put('/admin/company').send(newCompanyCredentials).expect(200);
    ...
    const pendingCompany = companies.companies.find(
      (company) => company.name === newCompanyCredentials.name,
    );
    this.companyID = pendingCompany.id;
  });

  it('creates a valid job using a valid admin account with a valid unofficial company id', function (done) {
    server
      .put(`/admin/company/${this.companyID}/jobs`)
      .set('Authorization', this.adminToken)
      .send({
  ...
  it('fails to add unofficial company when name already exists', function (done) {
    server
      .put('/admin/company')
      .send({
        location: 'Canberra',
