...
class Environment {
  ...
  private static parse(): EV {
    ...
    if (!preprocessed.success) {
      Logger.Error(`Couldn't process environment variables: ${JSON.stringify(preprocessed.error.format(), null)}`);
      process.exit(1);
    }
    ...
    const parsed: EV = {
      ...
      JOBS_BOARD_API_URL: preprocessed.data.JOBS_BOARD_API_URL,
    ...
    if (!validated.success) {
      Logger.Error(`Invalid environment variables: ${JSON.stringify(validated.error.format(), null)}`);
      process.exit(1);
    }
